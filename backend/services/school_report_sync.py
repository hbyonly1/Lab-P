...
async def _write_one_field(
    ...
    *,
    ...
    if target_type == "wysiwyg_image":
        return await _write_wysiwyg_image_field(page, modal_root, selector, value, source_id=source_id, timeout_ms=timeout_ms)
    if target_type != "text":
        raise _field_error(
...
async def _write_and_verify_fields(page: Any, config: Dict[str, Any], submission: Submission) -> Dict[str, List[Dict[str, Any]]]:
    modal_root = deep_get(config, "selectors.modal.root", "#ReportModal")
    ...
    for mapping in mappings:
        ...
        try:
            actual = await _write_one_field(page, modal_root, mapping, expected, timeout_ms=timeout_ms)
...
async def _click_submit_and_wait_feedback(page: Any, config: Dict[str, Any], mode: str) -> Dict[str, Any]:
    if mode == "draft":
    ...
    try:
        await page.wait_for_function(
            ...
            (feedbackScript) => {
              ...
              return messages.some((text) => /提交成功!?|保存成功!?|提交失败|保存失败|失败|错误|不正确/.test(text));
            }
            """,
            ...
            message=json.dumps(
                {
                    ...
                    "feedback": messages,
                    "matchedFailureMessages": [message for message in messages if re.search(r"失败|错误|不正确", message)],
    ...
    try:
        ...
        async def _run() -> Dict[str, Any]:
            async with school_session_manager.user_operation(user.id):
                ...
                field_write_report = await _write_and_verify_fields(page, config, submission)
                blocking_fields = (
                ...
                try:
                    feedback_result = await _click_submit_and_wait_feedback(page, config, mode)
                except SchoolAutomationError as exc:
